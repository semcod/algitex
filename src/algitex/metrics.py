...
class MetricsCollector:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = (
            Path(storage_path) if storage_path else Path(".algitex/metrics.json")
        )
    ...
    def estimate_cost(self) -> Dict[str, float]:
        ...
        for call in self.llm_calls:
            ...
            cost = (call.tokens_in / 1_000_000 * rate_in) + (
                call.tokens_out / 1_000_000 * rate_out
            )
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "fix_success_rate": sum(1 for f in self.fix_results if f.success)
            / len(self.fix_results)
            if self.fix_results
            else 0,
        }
    ...
    def save(self) -> None:
        ...
        data = {
            ...
            "llm_calls": [
                c.to_dict() for c in self.llm_calls[-1000:]
            ],  # Keep last 1000
...
class MetricsReporter:
    ...
    def print_dashboard(self, console=None) -> None:
        ...
        try:
            ...
            console.print(
                Panel(main_text, title="Algitex Metrics", border_style="blue")
            )
            ...
            print(
                f"LLM Calls: {summary['llm_calls']} (cached: {summary['cache_hit_rate']:.1%})"
            )
            print(
                f"Fixes: {summary['fix_attempts']} (success: {summary['fix_success_rate']:.1%})"
            )
