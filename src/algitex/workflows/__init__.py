...
class Pipeline:
    ...
    def __init__(self, path: str = ".", config: Optional[Config] = None):
        ...
        self.feedback_controller = FeedbackController(
            self.project.config.feedback_policy
            if hasattr(self.project.config, "feedback_policy")
            else None
        )
    ...
    def execute(self, max_tickets: int = 10, tool: str = "aider-mcp") -> Pipeline:
        ...
        self._steps.append(
            {"step": "execute", "tool": tool, "count": result["executed"]}
        )
        return self
...
class TicketExecutor:
    ...
    def __init__(
        ...
        self.feedback_loop = FeedbackLoop(
            feedback_controller, project._tickets, docker_mgr
        )
    ...
    def _call_tool_with_context(self, tool: str, ticket: Ticket, context) -> dict:
        ...
        if rt.tool.is_mcp:
            return self.docker_mgr.call_tool(
                tool,
                "aider_ai_code",
                {
    ...
    def _build_fix_prompt(self, ticket: Ticket) -> str:
        ...
        if ticket.meta.get("files_to_modify"):
            prompt += (
                f"Files to modify: {', '.join(ticket.meta['files_to_modify'])}\n\n"
            )
    ...
    def validate_all(self) -> dict:
        ...
        if "pytest" in self.docker_mgr.list_tools():
            runtime = self.docker_mgr.call_tool(
                "pytest",
                "python -m pytest /project/tests -v --tb=short --json-report",
                {},
            )
