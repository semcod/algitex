...
class ServiceChecker:
    ...
    def check_command_exists(self, name: str, command: str) -> ServiceStatus:
        ...
        try:
            result = subprocess.run(
                ["which", command], capture_output=True, text=True, timeout=self.timeout
            )

            if result.returncode == 0:
                path = result.stdout.strip()
                return ServiceStatus(
                    name=name, healthy=True, url=path, details={"path": path}
                )
            else:
                return ServiceStatus(
            ...
            return ServiceStatus(
                name=name, healthy=False, url="", error="Timeout checking command"
            )
        except Exception as e:
    ...
    def check_file_exists(self, name: str, path: str) -> ServiceStatus:
        ...
        if file_path.exists():
            ...
            return ServiceStatus(
                name=name, healthy=True, url=str(file_path), details=details
            )
        else:
            return ServiceStatus(
                name=name, healthy=False, url=str(file_path), error="File not found"
            )
    ...
    def check_all(
        ...
        for service_name, config in services.items():
            if service_name == "ollama":
                ...
                statuses.append(
                    self.check_command_exists(service_name, config["command"])
                )
            elif "path" in config:
    ...
    def print_status(
        self, statuses: List[ServiceStatus], show_details: bool = False
    ) -> None:
    ...
    def wait_for_services(
        ...
        while time.time() - start_time < timeout_seconds:
            ...
            all_healthy = all(
                service_map.get(s, ServiceStatus(s, False, "")).healthy
                for s in services
            )
