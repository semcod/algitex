...
class BenchmarkResults:
    ...
    def get_best_model(self, task_id: str, metric: str = "quality") -> Optional[str]:
        ...
        if metric == "quality":
            ...
            best = max(
                task_results, key=lambda r: r.quality_score / max(r.time_seconds, 0.1)
            )
...
for i in range(len(data)):
    ...
    def run_single_task(self, model: str, task: Task) -> TaskResult:
        ...
        try:
            response = self.client.generate(
                prompt=task.prompt, model=model, temperature=task.temperature
            )
        ...
        for model in models:
            ...
            for task_id in tasks_to_run:
                ...
                if progress:
                    if result.success:
                        print(
                            f"✅ {result.time_seconds:.1f}s, quality: {result.quality_score:.1f}/5"
                        )
                    else:
        ...
        for task_id in self.tasks.keys():
            ...
            for result in task_results:
                if result.success:
                    ...
                    rows.append([result.model, "FAILED", "-", "-", "-"])
            ...
            try:
                ...
                # Simple table format
                print(
                    f"{'Model':<25} {'Time':>8} {'Tokens':>8} {'Quality':>8} {'Tok/s':>8}"
                )
                print("-" * 100)
                for row in rows:
                    print(
                        f"{row[0]:<25} {row[1]:>8} {row[2]:>8} {row[3]:>8} {row[4]:>8}"
                    )
    ...
    def _print_detailed(self, results: BenchmarkResults):
        ...
        for result in results.results:
            ...
            print(
                f"Task: {result.tasks[result.task_id].name if result.task_id in self.tasks else result.task_id}"
            )
