...
class BatchFixBackend:
    ...
    def fix_batch(
        self, tasks: list[Task], max_parallel: int = MAX_3
    ) -> list[FixResult]:
        ...
        def process_group_with_progress(
            group_idx: int, group: TaskGroup
        ) -> list[FixResult]:
            nonlocal completed
    ...
    def _update_todo_mark_completed(
        self, tasks: list[Task], results: list[FixResult], elapsed: float
    ) -> None:
        ...
        print(
            f"     🔍 Szukam {len(completed_task_ids)} zadań w {len(todo_tasks)} taskach z TODO.md"
        )
        ...
        for task in todo_tasks:
            if task.id in completed_task_ids:
                ...
                print(
                    f"       ✓ Znaleziono do oznaczenia: {task.id} - {task.description[:CONSTANT_50]}..."
                )

        if not completed_todo_tasks:
            # Debug: pokaż przykłady ID które nie pasują
            print(
                f"       ⚠️  Przykłady completed_task_ids: {list(completed_task_ids)[:MAX_5]}"
            )
            print(
                f"       ⚠️  Przykłady task.id z TODO: {[t.id for t in todo_tasks[:MAX_5]]}"
            )
    ...
    def _verify_tasks_exist(self, tasks: list[Task]) -> list[Task]:
        ...
        for task in tasks:
            ...
            try:
                ...
                if "unused" in task.description.lower() or "unused_import" in str(
                    task.id
                ):
    ...
    def _group_tasks(self, tasks: list[Task]) -> list[TaskGroup]:
        ...
        BATCH_CATEGORIES = {
            ...
            "magic_number": ["magic number", "use named constant"],
            "unused_import": ["unused import", "unused"],
            "missing_return": ["missing return type", "return type"],
            "docstring": ["llm-style docstring", "docstring"],
            "module_block": ["module execution block", "if __name__"],
        }
    ...
    def _process_group(
        ...
        if total_groups > 0:
            print(
                f"  [{group_idx}/{total_groups}] 🔧 {group.category}: {len(group.tasks)} plików"
            )
        else:
        ...
        for f in group.files[:MAX_3]:
            ...
            rel_path = (
                ...
                if str(f).startswith(str(Path.cwd()))
                else f
            )
            print(f"     • {rel_path}")
    ...
    def _fix_individual(self, group: TaskGroup) -> list[FixResult]:
        ...
        for task in group.tasks:
            # Pokaż pełną ścieżkę dla łatwiejszego debugowania
            rel_path = (
                Path(task.file_path).relative_to(Path.cwd())
                if str(task.file_path).startswith(str(Path.cwd()))
                else task.file_path
            )
            ...
            print(
                f"       Wynik: {'✓ Sukces' if result.success else '✗ Porażka'} ({result.method})"
            )
            if result.error:
    ...
    def _call_llm(self, prompt: str, model: str, max_retries: int = 2) -> str:
        ...
        for attempt in range(max_retries + 1):
            ...
            while thread.is_alive():
                elapsed = time.time() - start
                sys.stdout.write(
                    f"\r      🤖 LLM: {model} {spinner[i % len(spinner)]} {elapsed:.0f}s/{self.timeout:.0f}s"
                )
                sys.stdout.flush()
                time.sleep(0.1)
                i += 1
    ...
    def _write_file_fix(
        ...
        try:
            ...
            if path.exists():
                ...
                print("         📦 Nowy plik (brak backup)")

            original = backups.get(filepath, "")
            if original == fixed_content:
                print("         ⚠️  Content nie zmieniony - pomijam zapis")
                return False
    ...
    def _validate_and_rollback(self, results: dict, backups: dict) -> None:
        ...
        try:
            ...
            if not hasattr(vallm, "validate_file"):
                print(
                    "     ⚠️  vallm.validate_file nie jest dostępne - pominięto walidację"
                )
                return
        except ImportError:
