...
class FallbackBackend(AutoFixBackend):
    ...
    def __init__(
        ...
        self.backends: Dict[str, BackendStatus] = {
            name: BackendStatus(name=name) for name in [primary] + self.fallbacks
        }
        ...
        verbose_print(
            f"FallbackBackend initialized: primary={primary}, fallbacks={self.fallbacks}"
        )
        ...
        if name not in self._backend_instances:
            ...
            if name == "litellm-proxy":
                self._backend_instances[name] = ProxyBackend(
                    proxy_url=self.proxy_url, dry_run=self.dry_run
                )
            elif name == "ollama":
                self._backend_instances[name] = OllamaBackend(
                    base_url=self.ollama_url, model=self.model, dry_run=self.dry_run
                )
            elif name == "aider":
                self._backend_instances[name] = AiderBackend(dry_run=self.dry_run)
            elif name == "openrouter":
                self._backend_instances[name] = OpenRouterBackend(dry_run=self.dry_run)
            else:
    ...
    def _try_backend(self, name: str, task: Task) -> Optional[FixResult]:
        ...
        try:
            ...
            if result.success:
                ...
                self._mark_failure(
                    name, f"Fix returned failure: {result.error or 'Unknown'}"
                )
    ...
    def fix(self, task: Task) -> FixResult:
        ...
        for attempt in range(self.retry_attempts):
            ...
            if attempt < self.retry_attempts - 1:
                print(
                    f"   ↻ Retrying {self.primary} (attempt {attempt + 2}/{self.retry_attempts})"
                )
                time.sleep(1)
        ...
        for fallback in self.fallbacks:
            ...
            for attempt in range(self.retry_attempts):
                verbose_print(
                    f"Fallback '{fallback}' attempt {attempt + 1}/{self.retry_attempts}"
                )
                ...
                if attempt < self.retry_attempts - 1:
                    print(
                        f"   ↻ Retrying {fallback} (attempt {attempt + 2}/{self.retry_attempts})"
                    )
                    time.sleep(1)
    ...
    def print_status(self):
        ...
        for name, status in self.backends.items():
            ...
            print(
                f"  {health} {name}: {status.success_count} OK, {status.fail_count} FAIL"
            )
