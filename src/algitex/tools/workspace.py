...
class Workspace:
        ...
        for name, repo in self.repos.items():
            for dep in repo.depends_on:
                if dep not in all_names:
                    raise ValueError(
                        f"Repo '{name}' depends on non-existent repo '{dep}'"
                    )
    ...
    def execute_all(
        self, tool: str = "aider-mcp", max_tickets: int = 5
    ) -> Dict[str, dict]:
    ...
    def status(self) -> dict:
        ...
        status = {
            "workspace": {
                "name": self.config.get("workspace", {}).get(
...
def create_workspace_template(name: str, repos: List[Dict]) -> str:
    ...
    template = {"workspace": {"name": name, "repos": repos}}
