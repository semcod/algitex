...
class BatchProcessor:
    ...
    def _process_item(self, item: Any, attempt: int = 1) -> BatchResult:
        ...
        try:
            ...
            return BatchResult(
                item=item, success=True, result=result, attempt=attempt, time_ms=elapsed
            )
            ...
            return BatchResult(
                item=item, success=False, error=str(e), attempt=attempt, time_ms=elapsed
            )
    ...
    def _execute(
        ...
        with ThreadPoolExecutor(max_workers=self.parallelism) as executor:
            # Submit all tasks
            future_to_item = {
                executor.submit(self._process_item, item): item for item in items
            }
            ...
            try:
                ...
                if pbar:
                    ...
                    pbar.set_postfix_str(
                        f"{status} {self.stats.successful}/{len(self.results)}"
                    )
                ...
                error_result = BatchResult(
                    item=item, success=False, error=str(e), time_ms=0
                )
...
class FileBatchProcessor(BatchProcessor):
    ...
    def process_directory(
        self, directory: Union[str, Path], pattern: str = "*.py", **kwargs
    ) -> List[BatchResult]:
