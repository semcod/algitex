...
class ConfigManager:
    ...
    def install_continue_config(
        self, models: Optional[List[str]] = None, config_dir: Optional[str] = None
    ) -> bool:
    ...
    def generate_vscode_settings(
        ...
        settings = {
            "github.copilot.enable": {"*": False, "yaml": True, "plaintext": True},
    ...
    def generate_env_file(
        self, services: Dict[str, str], env_path: Union[str, Path] = ".env"
    ) -> bool:
    ...
    def setup_project_configs(
        self, project_path: Union[str, Path] = ".", tools: List[str] = None
    ) -> bool:
        ...
        if "docker" in tools:
            ...
            success &= self.generate_docker_compose(
                services, project_path / "docker-compose.yml"
            )
