    ...
    def generate_gitlab_ci(self, output_path: Optional[str] = None) -> str:
        ...
        ci_config = {
            ...
            "analyze": {
                ...
                "before_script": ["pip install algitex code2llm vallm"],
                "script": [
...
CMD ["python", "-m", "algitex", "status"]
    ...
    def generate_precommit_config(self, output_path: Optional[str] = None) -> str:
        ...
        config_yaml = yaml.dump(
            precommit_config, default_flow_style=False, sort_keys=False
        )
...
def create_quality_gate_config(
    max_cc: float = 3.5, require_tests: bool = True, security_scan: bool = True
) -> Dict:
    ...
    config = {
        ...
        "env": {"LANGFUSE_DATABASE_URL": "${{ secrets.LANGFUSE_DATABASE_URL }}"},
        "steps": [
            {
