...
from clickmd import command, option
...
def metrics_cache(dir: str, list_entries: bool, clear_cache: bool):
    ...
    if list_entries:
        ...
        for e in entries[:50]:  # Show first 50
            table.add_row(
                e["hash"][:16], e["model"], str(e["age_hours"]), str(e["tokens"])
            )
...
def metrics_compare(storage: str):
    ...
    # Tokens
    algo_tokens = tier_stats.get("algorithm", {}).get("tokens_in", 0) + tier_stats.get(
        "algorithm", {}
    ).get("tokens_out", 0)
    micro_tokens = tier_stats.get("micro", {}).get("tokens_in", 0) + tier_stats.get(
        "micro", {}
    ).get("tokens_out", 0)
    big_tokens = tier_stats.get("big", {}).get("tokens_in", 0) + tier_stats.get(
        "big", {}
    ).get("tokens_out", 0)
