...
def todo_verify_prefact(file: str, prune: bool):
    ...
    try:
        ...
        console.print(
            "[red]✗ prefact not installed. Install with: pip install prefact[/]"
        )
        return
    ...
    if outdated_tasks:
        ...
        for task in outdated_tasks[:5]:
            console.print(
                f"   - {task['file']}:{task['line']} - {task['message'][:50]}"
            )
    ...
    if prune and outdated_tasks:
        ...
        console.print(
            f"[green]✓ Removed {len(outdated_tasks)} outdated tasks from {file}[/]"
        )
    elif prune:
