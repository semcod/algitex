...
def _extract_and_partition(
    project_path: Path, open_tickets, agents: int, verbose: bool
):
...
def _display_partition_plan(groups, open_tickets):
    ...
    for agent_idx, ticket_ids in groups.items():
        ...
        for tid in ticket_ids:
            ...
            title = (
                ticket.title if hasattr(ticket, "title") else ticket.get("title", tid)
            )
