...
class BenchmarkSuite:
    ...
    def summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_duration_sec": round(
                sum(r.duration_ms for r in self.results) / 1000, 2
            ),
    ...
    def print_table(self) -> None:
        ...
        for r in self.results:
            print(
                f"{r.name:<30} {r.duration_ms:>10.1f}ms {r.throughput:>10.1f}/s {r.memory_delta_mb:>8.2f}MB"
            )

        print(f"{'-' * 70}")
        summary = self.summary()
        print(
            f"Total: {summary['total']} benchmarks, {summary['total_duration_sec']}s, {summary['total_memory_mb']}MB"
        )
...
class BenchmarkRunner:
    ...
    def print_report(self) -> None:
        ...
        total_time = (
            sum(sum(r.duration_ms for r in s.results) for s in self.suites) / 1000
        )
        print(
            f"Suites: {len(self.suites)}, Benchmarks: {total_benchmarks}, Total time: {total_time:.1f}s"
        )
...
class CacheBenchmark:
    ...
    def bench_cache_hit_rate(
        cache_dir: str, entries: int = 1000, lookups: int = 5000
    ) -> Dict[str, float]:
    ...
    @staticmethod
    def bench_cache_deduplication(
        cache_dir: str, duplicates: int = 100
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "dedup_ratio": (duplicates / stats["entries"])
            if stats["entries"] > 0
            else float(duplicates),
...
def run_quick_benchmark() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        hit_result = CacheBenchmark.bench_cache_hit_rate(
            str(cache_dir), entries=100, lookups=500
        )
        dedup_result = CacheBenchmark.bench_cache_deduplication(
            str(cache_dir), duplicates=50
        )

        print(
            f"  Cache hit rate: {hit_result['hit_rate']:.1%} ({hit_result['throughput']:.0f} ops/sec)"
        )
        ...
        for tier, result in tier_results.items():
            throughput = result.get("throughput") or result.get(
